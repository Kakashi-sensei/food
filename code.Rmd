...
p <- ggplot(df_filtered, aes(x = Folate_mean, y = predicted_values)) +
  ...
  labs(
    ...
    color = "family history Status",  # Legend title for border color
    fill = "family history Status"    # Legend title for fill color
  ) +
...
```{r}
#trouble shooting interaction test for significant figures

# example -- IC1 with Folate on FH
library(lmerTest)
df<-read.csv(file = 'C:\\Users\\timothy\\OneDrive\\Documents\\ISU\\food_project\\data\\data_for_test\\food_continuous.csv')

# Fit the model -- factor all factor variables
factor_vars <- c("AlcoholY08", "SmokingY08", "Assessment_0", "EID", "APOE_all", "T40_Gcar", "family_history_AD", "Sex")
for (var in factor_vars) {
  df[[var]] <- as.factor(df[[var]])
}
rm(factor_vars,var)
...
rm(scale_vars, var)
# exclude cases with NA (missing) values

vars_of_interest <- c("AlcoholY08", "SmokingY08", "Assessment_0", "EID", "APOE_all", "T40_Gcar", "family_history_AD", "Sex", "VB6_mean", "VB12_mean", "Folate_mean", "rsfMRI_Component1", "rsfMRI_Component5", "rsfMRI_Component7", "rsfMRI_Component9", "rsfMRI_Component10", "rsfMRI_Component13", "rsfMRI_Component14", "rsfMRI_Component20", "Age_Baseline", "BMI_Y08andY12andY14_mean", "Townsend")

df_no_na <- df[!apply(df[vars_of_interest], 1, function(x) any(is.na(x))), ]
